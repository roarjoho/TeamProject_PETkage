		...
		<div class="container">			
			<div class="row">
				...
				<div class="col-lg-9 order-1 order-lg-2">
					<div class="container">
					<div>
					<h3 style="float: left;">내 정보공유글 보기</h3>
					<select id="boardSelectOption" onchange="location.href=this.value" style="float: right;">
						...
						<option selected="selected" value="${pageContext.request.contextPath}/mypage/mypageBoard3">정보공유</option>
						<option value="${pageContext.request.contextPath}/mypage/mypageBoard4">자랑(사진)</option>
					</select>
					</div>
								<div class="cart-table">
			                        <table>
			                            ...
			                            <tbody>
			                            	<c:if test="${myBoardView3 ne null }">
				                            	<c:forEach var="myBoardView3" varStatus="status" items="${myBoardView3}">
				                             		<tr>
				                             			<td>${status.count }</td>
				                             			<td>${myBoardView3.board_title}</td>
				                             			<td>${myBoardView3.user_id}</td>
				                             			<td>${myBoardView3.board_createDate}</td>
				                             			<td>${myBoardView3.board_hit}</td>
				                             		</tr>
				                             	</c:forEach>				                           
			                             	</c:if>
